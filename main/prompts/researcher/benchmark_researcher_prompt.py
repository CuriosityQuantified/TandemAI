"""
Benchmark Researcher/Fact-Finding Agent System Prompt - Enhanced V3

VERSION: 3.0 (Baseline Benchmark)
DATE CREATED: 2025-11-13
KNOWN ISSUES:
  - GraphRecursionError: Enhanced sections (V4+) caused infinite recursion at 25-step limit
  - Restoration attempts failed at 0-24% completion
  - Token count: See get_token_count() for current measurement

BASELINE METRICS:
  - This is the benchmark prompt for evaluation framework
  - All challenger prompts will be compared against this version
  - Statistical significance: N≥32 test queries required
...
# ==============================================================================
# PROMPT CONTENT (Original Enhanced V3)
# ==============================================================================
...
═══════════════════════════════════════════════════════════════════════════
🚨 MANDATORY: PLANNING DECISION TREE (READ THIS FIRST) 🚨
═══════════════════════════════════════════════════════════════════════════
...
def count_prompt_tokens(
    ...
    # We count system prompt tokens by passing it as a system message
    response = client.beta.messages.count_tokens(
        model=model,
        system=prompt,
        messages=[{"role": "user", "content": "test"}]  # Minimal message to trigger counting
    )

    # Return input_tokens which includes the system prompt
...
def get_prompt_metadata() -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with version info, token count, known issues, etc.
    ...
    return {
        "version": "3.0",
        "name": "Enhanced V3 (Baseline Benchmark)",
        ...
        "known_issues": [
            "GraphRecursionError: Enhanced sections (V4+) caused infinite recursion at 25-step limit",
            "Restoration attempts failed at 0-24% completion",
            "This is the stable baseline for comparison"
        ],
        "features": [
            "Autonomous execution with planning decision tree",
            "Task completion verification",
            "Sequential execution pattern",
            "Extensive citation protocol",
            "Progress tracking with update_plan_progress",
...
if __name__ == "__main__":
    print("=" * 80)
    print("BENCHMARK RESEARCHER PROMPT - TOKEN COUNT ANALYSIS")
    print("=" * 80)
    print()
