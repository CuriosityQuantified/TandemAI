"""
Run Full Kimi K2 Evaluation (32 queries) on Researcher v3.1 - PARALLEL EXECUTION

This script runs all 32 test queries in parallel using Kimi K2 Thinking model via Groq.
Uses the best-performing v3.1 researcher prompt with asyncio for improved speed.

Model: moonshotai/kimi-k2-instruct-0905 (Groq)
Prompt: v3.1 (challenger, best citation accuracy)
Judge: gemini-2.5-flash (same as baseline/challenger for fair comparison)

Usage:
    python evaluation/run_parallel_kimi_k2.py

Expected runtime: ~15-25 minutes
Note: max_concurrency=5 (Groq has different rate limits than Gemini)
"""
...
cache_dir = Path(__file__).parent.parent / "backend" / "prompts" / "versions" / "researcher" / "__pycache__"
if cache_dir.exists():
    shutil.rmtree(cache_dir)
    print("✓ Cleared Python bytecode cache")

from evaluation.test_suite import get_test_suite
from evaluation.agent_invoker import async_invoke_researcher_batch_groq
from evaluation.judge_integration import run_evaluation_for_query
from evaluation.rubrics import EvaluationResult
from backend.prompts.versions.researcher.v3_1 import get_researcher_prompt as get_v3_1_prompt


async def run_parallel_kimi_k2_evaluation(
    max_concurrency: int = 5,
    save_results: bool = True
):
    """
    Run all 32 queries on researcher v3.1 with Kimi K2 model in parallel.

    Args:
        max_concurrency: Maximum concurrent agent executions (default: 5 for Groq)
    ...
    print("=" * 80)
    print("PARALLEL KIMI K2 EVALUATION - Researcher v3.1 + Kimi K2 Thinking")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  - Total queries: 32")
    print(f"  - Max concurrency: {max_concurrency}")
    print(f"  - Prompt version: v3.1 (challenger)")
    print(f"  - Agent model: {model_name} (Groq)")
    ...
    # Step 2: Run all agents in parallel with Kimi K2
    print("Step 2: Running all 32 agent executions in parallel with Kimi K2...")
    print(f"{'=' * 80}\n")

    start_time = datetime.now()

    responses = await async_invoke_researcher_batch_groq(
        prompt_func=get_v3_1_prompt,
        queries=queries,
    ...
    for i, (query_obj, response) in enumerate(zip(test_suite, responses)):
        ...
        try:
            ...
            # Set metadata
            eval_result.prompt_version = "researcher_v3.1_kimi_k2"
    ...
    if save_results and results:
        output_dir = Path(__file__).parent / "experiments" / f"kimi_k2_v3.1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        results_data = {
            "metadata": {
                "prompt_version": "researcher_v3.1",
...
if __name__ == "__main__":
    print("\n🚀 Starting parallel Kimi K2 evaluation...\n")

    # Run async main function
    results = asyncio.run(run_parallel_kimi_k2_evaluation(
        max_concurrency=5,  # Groq rate limits differ from Gemini
        save_results=True
    ))
