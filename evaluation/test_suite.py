...
def get_test_suite() -> List[TestQuery]:
    """
    Get the complete test suite (all 32 queries).

    Returns:
        List of TestQuery objects (32 total across 4 categories):
        - 8 Simple queries (3-4 steps)
        - 8 Multi-aspect queries (5-6 steps)
        - 8 Time-constrained queries (recent information)
        - 8 Comprehensive queries (7-10 steps)
...
def run_single_test(
    ...
    try:
        ...
        # Extract response (handle both string and list content)
        messages = result.get("messages", [])
        if messages:
            content = messages[-1].content
            # Handle multimodal content (list) vs text content (string)
            if isinstance(content, list):
                ...
                agent_response = str(content) if content else ""
        else:
            agent_response = ""
