"""
Run Full Challenger Evaluation (32 queries) on Researcher v3.1 - PARALLEL EXECUTION
...
Usage:
    python evaluation/run_parallel_challenger.py
...
async def run_parallel_challenger_evaluation(
    max_concurrency: int = 2,
    save_results: bool = True
):
    """
    Run all 32 queries on researcher v3.1 in parallel.
    ...
    """
    print("=" * 80)
    print("PARALLEL CHALLENGER EVALUATION - Researcher v3.1")
    print("=" * 80)
    print(f"\nConfiguration:")
    ...
    for i, (query_obj, response) in enumerate(zip(test_suite, responses)):
        ...
        try:
            ...
            # Set metadata
            eval_result.prompt_version = "researcher_v3.1"
    ...
    if save_results and results:
        output_dir = Path(__file__).parent / "experiments" / f"challenger_v3.1_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
if __name__ == "__main__":
    print("\n🚀 Starting parallel challenger evaluation...\n")

    # Run async main function
    results = asyncio.run(run_parallel_challenger_evaluation(
