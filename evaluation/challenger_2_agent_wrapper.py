...
This module provides a wrapper to integrate test_config_challenger_2.py
...
BASELINE CONFIGURATION (Researcher Agent):
    ...
    - Config: test_config_challenger_2.py
...
# Add necessary paths
root_path = Path(__file__).parent.parent  # evaluation/ → TandemAI/
sys.path.insert(0, str(root_path))  # Add TandemAI root for absolute imports
...
def extract_plan_from_messages(messages: list) -> Dict[str, Any] | None:
    ...
    for msg in messages:
        if isinstance(msg, ToolMessage) and msg.name == "create_research_plan":
            ...
            # Tool returns: "✅ Research plan created with {num_steps} steps:\n{json_object}"
            # We need to extract the JSON part (everything after the first newline)
            import json
            try:
                content = str(msg.content)

                # Find the JSON part (after first newline)
                if '\n' in content:
                    json_part = content.split('\n', 1)[1]
                else:
                    # If no newline, try the whole content
                    json_part = content
                ...
                print(f"Warning: Failed to parse plan from tool message: {e}")
                return None
    return None
...
def create_baseline_agent(agent_type: AgentType) -> callable:
    ...
    BASELINE CONFIGURATION DETAILS (Researcher):
        - Configuration: test_config_challenger_2
    ...
    from evaluation.configs.test_config_challenger_2 import graph
    ...
    def execute_research_query(query: str) -> Dict[str, Any]:
        ...
        Configuration: test_config_challenger_2 (Researcher)
...
def get_baseline_agent(agent_type: AgentType = "Researcher") -> callable:
    ...
    if agent_type not in _baseline_agent_instances:
        ...
        print(f"  Config: test_config_challenger_2")
...
if __name__ == "__main__":
    ...
    print("  - Config: test_config_challenger_2")
